...
class OCRApp:
    ...
    def process_image(self, image_path, output_path, lang, format):
        ...
        output_file_path = os.path.join(output_path, f"{base_filename}.{format}")
        ...
        with self.lock:
            completed_percentage = (self.progress_var.value + (1.0 / len(images))) * 100
            self.progress_var.value += 1.0 / len(images)
            self.progress_label_var.set(f"{int(completed_percentage)}%")
